...
class DispatchConfig:
    ...
    geo_capacity_mwth: float = 10.12      # geothermal thermal capacity to district
...
def sizing_table(capacities=(5.05, 7.5, 10.12), cfg: DispatchConfig | None = None) -> pd.DataFrame:
...
def lcoe_at_simulated_loadhours(cfg: DispatchConfig | None = None) -> dict:
    ...
    sc = SchemeConfig()
    assumed = heat_economics(sc.heat_case(), heat_mwth=sc.heat_mwth_p50)
    derived_case = sc.heat_case()
    from dataclasses import replace
    derived_case = replace(derived_case, heat_loadhours=s["geo_fleq_h"])
    derived = heat_economics(derived_case, heat_mwth=sc.heat_mwth_p50)
    return {
...
def plot_dispatch(cfg: DispatchConfig | None = None, save_path=None):
    ...
    base = np.minimum(ldc, 5.05)
    ax1.fill_between(x, 0, base, color="#c1432b", alpha=0.85, label="1 doublet baseload (~5 MWth)")
    ax1.axhline(5.05, color="#1F4E5F", lw=1, ls="--")
    ax1.axhline(10.12, color="#3b7dab", lw=1, ls="--", label="2 doublets (peak-sized)")
    ax1.set_xlabel("% of year (load-duration)")
    ax1.set_ylabel("MWth")
    ax1.set_title("Heat load-duration — geothermal baseload vs peak")
    ax1.legend(fontsize=8, loc="upper right")

    st = sizing_table(capacities=(5.05, 6.0, 7.5, 8.5, 10.12), cfg=cfg)
...
if __name__ == "__main__":
    ...
    print("=== dispatch summary (geo = 10.12 MWth) ===")
