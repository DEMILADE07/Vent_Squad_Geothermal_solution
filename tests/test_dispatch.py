...
def test_peak_sized_geothermal_is_underutilised():
    ...
    s = simulate_dispatch(DispatchConfig(geo_capacity_mwth=10.0))
...
def test_baseload_sizing_recovers_the_assumed_loadhours():
    ...
    st = sizing_table(capacities=(5.06, 10.0)).set_index("geo_capacity_mwth")
    assert st.loc[5.06, "geo_fleq_h"] > 5000
    assert st.loc[5.06, "geo_coverage_pct"] > 85.0
    assert st.loc[5.06, "geo_fleq_h"] > st.loc[10.0, "geo_fleq_h"]   # monotone
...
def test_ates_pair_count_is_validated_by_peak_cooling():
    ...
    assert c["ates_pairs"] == 5                  # ceil(5 / 1.0) at the conservative throughput
