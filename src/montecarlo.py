...
from src.constants import (
    ATES_THROUGHPUT_MAX_MWTH_PER_PAIR,
    ATES_THROUGHPUT_MIN_MWTH_PER_PAIR,
    ATES_THROUGHPUT_MODE_MWTH_PER_PAIR,
    DEMAND_COOLING_MWTH,
...
def ates_cooling_adequacy(n_pairs=6, demand_mwth=DEMAND_COOLING_MWTH, n=100_000,
                          seed=20260607) -> dict:
    """Probabilistic cooling adequacy for an ``n_pairs`` ATES scheme.

    Each warm/cold pair delivers an uncertain 0.5-2.0 MWth of cooling (triangular,
    mode 1.0); pairs are independent. We return the supply distribution and the
    probability of meeting the cooling demand, so the cooling side carries the same
    uncertainty treatment as the heat side rather than a single midpoint pick.
    """
    rng = np.random.default_rng(seed)
    per_pair = rng.triangular(
        ATES_THROUGHPUT_MIN_MWTH_PER_PAIR,
        ATES_THROUGHPUT_MODE_MWTH_PER_PAIR,
        ATES_THROUGHPUT_MAX_MWTH_PER_PAIR,
        size=(n, n_pairs),
    )
    supply = per_pair.sum(axis=1)
    return {
        "n_pairs": n_pairs,
        "demand_mwth": demand_mwth,
        "supply_p90": round(float(np.percentile(supply, 10)), 2),  # conservative
        "supply_p50": round(float(np.percentile(supply, 50)), 2),
        "supply_p10": round(float(np.percentile(supply, 90)), 2),  # optimistic
        "supply_mean": round(float(supply.mean()), 2),
        "p_meets_demand": round(float((supply >= demand_mwth).mean()), 3),
    }


def summarise(mc: pd.DataFrame, demand_mwth=10.0, n_doublets=1,
              seed=20260607) -> pd.DataFrame:
    """Per-well P10/P50/P90 MWth and P(scheme >= demand) for an n_doublets scheme.

    A k-doublet scheme is modelled as the SUM of k *independent* single-doublet
    realisations drawn from the same ThermoGIS-anchored band. Two doublets in the
    same play share the regional resource distribution but not one identical
    reservoir realisation, so we bootstrap k independent resamples of the
    single-doublet draws and sum them — rather than rescaling one draw by k.

    The rescaling shortcut (scheme = k * single_draw) imposes perfect correlation,
    which forces P(2X >= D) = P(X >= D/2) and *understates* the genuine two-site
    result: summing independent draws reduces skew, so the scheme median rises
    faster than k * median. For k=1 this reduces to the single-doublet distribution.
    """
    rng = np.random.default_rng(seed)
    rows = []
    for well, g in mc.groupby("well", sort=False):
        ...
        if n_doublets <= 1:
            ...
            scheme = np.zeros(x.size)
            for _ in range(n_doublets):
                scheme = scheme + rng.choice(x, size=x.size, replace=True)
        rows.append({
            "well": well,
