...
def financed_lcoe(capex_mln: float, annual_opex_eur: float, annual_energy_gj: float,
    ...
    annual_opex_eur    positive fixed+variable O&M per operating year.
    annual_energy_gj   energy delivered per operating year (GJ).
    ...
    energy_gj = np.where(operating, annual_energy_gj, 0.0)
    opex = np.where(operating, -annual_opex_eur, 0.0)
...
def heat_economics(c: HeatCase, heat_mwth: float | None = None) -> dict:
    ...
    annual_opex = heat_mwth * (c.heat_loadhours * om_var_eur_mwh
                               + om_fixed_keur_mwth * 1000.0)
    ...
    res = financed_lcoe(capex_mln, annual_opex, annual_energy_gj, c.fin,
                        lifetime_yr=c.fin.term_loan_yr, reinvest=reinvest)
    res.update({
        "product": "heat",
        "heat_mwth": heat_mwth,
        "flow_ls": flow_ls,
