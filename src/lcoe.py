...
def financed_lcoe(capex_mln: float, annual_opex_eur: float, annual_energy_gj: float,
    ...
    for _name, _arr in (("annual_energy_gj", annual_energy_gj),
                        ("annual_opex_eur", annual_opex_eur)):
        _n = np.asarray(_arr, float).size
        if _n not in (1, horizon):
            raise ValueError(
                f"{_name} must be a scalar or a per-year array of length "
                f"horizon={horizon} (got length {_n}); a thermal-decline profile "
                f"should cover all {horizon} cash-flow years.")
...
class CoolingCase:
    ...
    # 6 warm/cold pairs, not 4: a single ATES pair delivers ~0.5-2.0 MWth of cooling
    # (Bloemendal & Hartog 2018; Fleuchaus et al. 2018, Renew. Sustain. Energy Rev.),
    # so sizing on the optimistic ~1.5 midpoint left the design failing 5 MWth at the
    # low end. At a conservative 1.0 MWth/pair central, 6 pairs give a P50 supply of
    # ~6.8 MWth and P(supply >= 5 MWth) ~= 0.998 (src/montecarlo.ates_cooling_adequacy).
    ates_pairs: int = 6                    # warm/cold well pairs
    ates_capex_mln_per_pair: float = 0.7   # shallow ATES doublet + HX (NL range 0.5-0.8)
    ates_throughput_mwth_per_pair: float = 1.0   # conservative central (mode of 0.5-2.0)
    ates_round_trip_eff: float = 0.70
    chiller_capex_keur_mwth: float = 120.0  # trim heat-pump / chiller plant
    # Blended seasonal COP: ATES supplies most cold by circulation only (COP ~20-30,
    # pumping energy only; Fleuchaus et al. 2018), an electric heat pump trims the
    # summer peak at COP ~4.5 (typical chiller SPF). At a ~70/30 ATES/HP energy split
    # the blended seasonal COP is ~10; the tornado sweeps this down to 6 as downside.
