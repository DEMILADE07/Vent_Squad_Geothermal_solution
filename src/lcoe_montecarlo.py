...
def simulate_lcoe(cfg: SchemeConfig | None = None,
    ...
    Resource MWth is the bounded 2-doublet Monte-Carlo (split-lognormal + pump cap);
    each resource draw is paired with an independent cost/market draw. Every draw
    assumes the four-well scheme is built and delivers whatever the resource gives —
    so the LCOE upper tail is exactly the cost of the resource under-delivering.
    """
    cfg = cfg or SchemeConfig()
    unc = unc or LcoeUncertainty()
    rng = np.random.default_rng(seed)

    # Resource: single-doublet draws at the anchor well, scaled to the scheme.
    df_tg = load_thermogis()
    res = simulate_well(df_tg, anchor_well, n=n, seed=seed + 101)
    scheme_mwth = res["mwth"].to_numpy() * cfg.n_doublets
    ...
    for i in range(n):
        ...
        kc = CoolingCase(
            cooling_mwth=cfg.demand_cooling_mwth, ates_pairs=4,
